#!/usr/bin/env python3
"""
Stage 2: Generate Videos from Images - ENGLISH VERSION (Character Voices)
Reads generated images from Stage 1 and English video prompts with character voices,
then generates videos using LTX2 with classical music background.

Uses: 02_video_prompts_english_optimized.txt (120-150 words, multiple character voices)

Character voices include:
- NARRATOR: Soft warm maternal voice
- LUMI: Young innocent child voice
- WISE OWL: Deep gentle grandfatherly voice
- CRICKET: Cheerful friendly voice
- MOTHER BUNNY: Warm loving adult female voice
...
class EnglishVideoGenerator:
    ...
    def load_video_prompts(self, story_path: Path) -> List[str]:
        """Load English video prompts from file."""
        prompts_file = story_path / '02_video_prompts_english_optimized.txt'

        if not prompts_file.exists():
            raise FileNotFoundError(f"English video prompts file not found: {prompts_file}")
    ...
    def queue_video(self, image_filename: str, prompt: str, scene_number: int, output_prefix: str) -> str:
        """Queue a single video generation with English character voices."""
        workflow = {
            "1": {
                "inputs": {
            ...
            "99": {
                "inputs": {
                    "filename_prefix": f"{output_prefix}_english_scene_{scene_number:03d}",
                    "fps": 24,
                    "compress_level": 4,
                    "images": ["98", 0]
                },
        ...
        payload = {
            "prompt": workflow,
            "client_id": f"english_video_{scene_number}"
        }
    ...
    def generate_story(self, story_name: str):
        """Generate all English videos for a story."""
        story_path = self.get_story_path(story_name)
        ...
        print(f"\n📖 Processing: {story_path.name} (ENGLISH)")
        print(f"   Path: {story_path}")

        # Load English video prompts
        try:
            video_prompts = self.load_video_prompts(story_path)
            print(f"   Found {len(video_prompts)} English video prompts")
        ...
        print("="*70)
        print(f"🎬 Generating English Videos for {story_path.name}")
        print(f"   Voices: Multiple characters (NARRATOR, LUMI, WISE OWL, etc.)")
        ...
        print("\n" + "="*70)
        print(f"✨ Complete - {story_path.name} (ENGLISH)")
        print("="*70)
        print(f"   ✅ Queued: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📁 Output: C:/ComfyUI/output/")
        print(f"   🎵 Features: Character voices + Classical music")
        print("="*70 + "\n")
...
def main():
    ...
    print("\n" + "="*70)
    print("  🎬 Stage 2: English Video Generation (LTX2)")
    print("  🗣️  Character Voices Version")
    print("="*70 + "\n")

    if len(sys.argv) < 2:
        print("Usage:")
        print("  python 02_generate_videos_english.py <story_name>")
        print("\nExamples:")
        print("  python 02_generate_videos_english.py lumi_bunny")
        print("  python 02_generate_videos_english.py bruno_bear")
        print("  python 02_generate_videos_english.py all")
        print("\nAvailable stories:")
        ...
        print("\n📝 This version uses English character voices")
        print("   For Spanish narrator, use: 02_generate_videos_spanish.py")
        print("\n🎭 Character Voices:")
        print("   - NARRATOR: Soft warm maternal voice")
        print("   - LUMI: Young innocent child voice")
        print("   - WISE OWL: Deep gentle grandfatherly voice")
        print("   - CRICKET: Cheerful friendly voice")
        print("   - And more depending on the story!")
        print()
        sys.exit(1)
